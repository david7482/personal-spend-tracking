...
- **職責**：
  ...
  3. 抽取 sender、subject（自動解碼 MIME RFC 2047 編碼）、body (HTML/text)
...
- **IaC 工具**：Terraform（S3 backend, no DynamoDB locking）
- **Terraform providers**：AWS (`~> 5.0`) + Cloudflare (`~> 5.0`)
- **DNS 管理**：Cloudflare Terraform provider（SES TXT verification + MX record）
- **部署 region**：`us-east-1`（SES Inbound 要求）
- **Lambda 部署**：`make deploy`（Makefile + AWS CLI `update-function-code`）
- **DB 遷移**：Alembic（raw SQL migrations, `make migrate`）
- **本地密鑰**：`.envrc`（direnv）管理 `CLOUDFLARE_API_TOKEN` 和 `DATABASE_URL`
